...
class Apache5RestResponse<T> implements RestResponse<T> {
    ...
    public static <T> Apache5RestResponse<T> create(
        RestParser parser,
        ...
        if (statusCode >= 400) {
            try (InputStream in = entity.getContent()) {
                throw new RestResponseException(
                    RestUtils.toErrorResult(parser, statusCode, statusMessage, in),
                    statusCode,
                    statusMessage,
                    null
                );
        ...
        return new Apache5RestResponse<>(
            statusCode,
            statusMessage,
            headers,
            contentType != null ? MediaType.parse(contentType.toString()) : null,
            contentLength,
            body
        );
    }
